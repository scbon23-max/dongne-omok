create or replace function public.holdem_ring_refill_v3_compare_and_swap(
  ...
  p_nickname text,
  p_anycall_event_refill boolean default false
)
returns table (
  applied boolean,
  reason text,
...
declare
  ...
  big_blind bigint;
  expected_refill_amount bigint;
  minimum_playable_assets bigint := 20000;
  wallet_balance bigint;
  table_assets bigint;
  total_assets bigint;
begin
  refill_amount := nullif(p_state #>> '{settings,refillAmount}', '')::bigint;
  big_blind := nullif(p_state #>> '{settings,bigBlind}', '')::bigint;
  expected_refill_amount := case
    when p_anycall_event_refill is true
      then greatest(100, coalesce(big_blind, 0)) * 100
    else 20000
  end;

  if p_anycall_event_refill is true and coalesce(big_blind, 0) < 100 then
    raise exception 'invalid holdem anycall refill blind'
      using errcode = '22023';
  end if;

  if p_anycall_event_refill is not true
      and refill_amount is distinct from expected_refill_amount then
  ...
  if total_assets >= minimum_playable_assets
      and p_anycall_event_refill is not true then
    return query select false, 'assets_remaining'::text, current_table.state,
      current_table.version, current_table.owner_nickname, 0, 0;
    return;
  end if;

  if p_anycall_event_refill is true then
    ...
    if not found then
      raise exception 'holdem anycall refill table update conflict'
        using errcode = '40001';
    end if;
    ...
    return query select true, null::text, current_table.state,
      current_table.version, current_table.owner_nickname, 0, 3;
    return;
  end if;
