...
## 🌐 호스팅 (2026-07-13 이전 완료 — 중요)
- **Netlify → GitHub Pages로 이전 완료.** 이유: Netlify 크레딧 소진으로 프로덕션 배포 정지됨.
- **새 사이트 주소: https://scbon23-max.github.io/dongne-omok/** (실측 정상 동작 확인). 옛 Netlify 주소는 폐기 예정 → 클럽에 새 주소 공지 필요.
- **저장소: https://github.com/scbon23-max/dongne-omok** (공개 repo, 슈팅게임 repo와 완전 별개). Supabase 공개키라 공개해도 안전.
- **앞으로 배포법 = `git push`**: `dongne-omok/`에서 `git add -A && git commit -m "..." && git push`. (드래그 아님. 로컬은 git repo가 됨, main 브랜치.) Pages가 push 후 1~2분 내 자동 반영.
- ⚠️ Supabase는 그대로(URL/키/채널 `main` 동일). 새 도메인에서 실시간·랭킹이 정상인지 **2인 대국으로 한 번 실검 권장**(anon키+RLS라 도메인 제약 없을 것으로 보이나 미검증).

## ⚠️ 지금 당장 할 일 (제일 중요)
1. ~~**재배포**~~ ✅ **완료** — 이번 세션 모든 변경이 GitHub Pages 새 주소에 반영됨(아래 목록 전부 라이브). 옛 Netlify는 신경 안 써도 됨.
2. **참고용 변경 이력(이미 배포됨, 로컬만 아님)**:
