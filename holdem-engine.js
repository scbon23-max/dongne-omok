(function (root, factory) {
  ...
  function ensureAdditiveState(state, now, randomInt) {
    ...
    state.seats.forEach(function (player, seatIndex) {
      ...
      player.topUpSkippedHandNo = Math.max(0, integer(player.topUpSkippedHandNo, 0));
  ...
  function settleReservedTopUps(state, context) {
    if (!state || !state.settings || state.settings.mode !== "ring") return;
    state.seats.forEach(function (player) {
      ...
      var balances = context && context.topUpBalances;
      if (delta > 0 && state.settings.assetBacked === true && balances &&
          delta > Math.max(0, integer(balances[player.nick], 0))) {
        player.topUpReserved = false;
        player.topUpTargetAmount = 0;
        player.topUpAppliedHandNo = 0;
        player.topUpSkippedHandNo = Math.max(0, integer(state.handNo, 0)) + 1;
        return;
      }
      if (delta > 0) {
        player.stack = target;
  ...
  function recordPlayerTimeout(player) {
    if (!player || player.isBot) return false;
    player.timeoutStreak = clamp(
      integer(player.timeoutStreak, 0) + 1,
      0,
      AUTO_SIT_OUT_TIMEOUTS,
      AUTO_SIT_OUT_TIMEOUTS
    );
    if (player.timeoutStreak >= AUTO_SIT_OUT_TIMEOUTS) {
  ...
  function finishHandPlayers(state) {
    ...
    state.seats.forEach(function (player, seat) {
      ...
      else if (!player.leaving && !player.away && !player.sittingOut) player.ready = true;
  ...
  function startHand(state, now, context) {
    ...
    settleReservedTopUps(state, context);
  ...
  function applyPresenceState(state, cmd, now) {
    ...
    state.seats.forEach(function (player) {
      ...
      else if (!PLAYING_PHASES[state.phase] && !player.leaving &&
          !player.sittingOut && player.stack > 0) player.ready = true;
    });
      ...
      if (type === "join") {
        if (!nick) result = { ok: false, reason: "nick" };
          ...
          if (requestedMoveSeat >= 0 && requestedMoveSeat < MAX_SEATS && requestedMoveSeat !== player.seat) {
            ...
            player.leaving = false;
            player.leavingIntent = "";
            var resumedFromTimeout = resetPlayerTimeoutState(next, player);
            result = { ok: true, reason: resumedFromTimeout ? "resumed" : "already_joined" };
            changed = true;
          }
        ...
        if (!player) result = { ok: false, reason: "not_joined" };
          ...
          var nextReady = cmd.ready == null ? !player.ready : !!cmd.ready;
          if (nextReady) resetPlayerTimeoutState(next, player);
        ...
        if (!PLAYING_PHASES[next.phase] || next.actorSeat == null || next.actionDeadline == null || now < next.actionDeadline) {
          ...
          if (result.ok) {
            recordPlayerTimeout(player);
            next.lastEvent.timeout = true;
            next.lastEvent.timeoutStreak = player.isBot ? 0 : player.timeoutStreak;
            next.lastEvent.sittingOut = player.isBot ? false : player.sittingOut === true;
            changed = true;
          }
        }
      } else {
  ...
  function publicPlayer(player, viewerPlayer, revealAll) {
    ...
    var out = {
      ...
      topUpTargetAmount: player.topUpReserved ? Math.max(0, integer(player.topUpTargetAmount, 0)) : 0,
      topUpAppliedHandNo: Math.max(0, integer(player.topUpAppliedHandNo, 0)),
      topUpSkippedHandNo: Math.max(0, integer(player.topUpSkippedHandNo, 0)),
  ...
  function safeEvent(event) {
    ...
    var allowed = [
      ...
      "grossAmount", "rake", "fullRaise", "timeout", "timeoutStreak", "sittingOut",
      "winners", "reason", "away", "at", "botId",
  ...
  function buildView(state, viewerPlayer, viewerNick, canManageBots) {
    ...
    var canStart = !PLAYING_PHASES[state.phase] && state.phase !== "tournament_end" &&
      ...
      !viewerPlayer.away && !viewerPlayer.sittingOut &&
