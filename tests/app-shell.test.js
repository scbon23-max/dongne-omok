...
test("room creation keeps Hold'em public and joinable from the list", () => {
  ...
  assert.match(index, /id="create-holdem-buyin-slider"[^>]*type="hidden"[^>]*min="10000"[^>]*max="100000"[^>]*step="100"[^>]*value=""/);
  ...
  assert.match(styles, /\.create-room-dialog\s*\{[^}]*radial-gradient\(ellipse at 50% -12%, rgba\(139,255,215,\.18\)/);
  assert.match(styles, /\.create-room-dialog\s*\{[^}]*repeating-linear-gradient\(115deg/);
