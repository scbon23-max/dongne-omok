...
test("the Pro difficulty routes only its moves through Rapfi", () => {
  ...
  assert.match(game, /rapfi-worker\.js\?v=rapfi-3aedf3a-depth-v1-20260717/);
  ...
  assert.match(game, /var PRO_DEPTH_30_SECONDS = 15/);
  assert.match(game, /var PRO_DEPTH_60_SECONDS = 17/);
  assert.match(game, /var PRO_DEPTH_120_SECONDS = 19/);
  assert.match(game, /var PRO_UNLIMITED_DEPTH = 21/);
  assert.match(game, /maxDepth: proSearchDepth\(\),\s*depthTarget: true/);
...
test("every Pro game explains the timer-strength relationship in room chat", () => {
  ...
  assert.match(game, /30초는 깊이 15, 1분은 17, 2분은 19, 무한은 21이며 목표 깊이를 완주하면 바로 둡니다/);
});
