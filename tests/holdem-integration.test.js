...
test("the six-seat table exposes every required game control", () => {
  ...
  assert.match(classicCurrentRule, /border:\s*1px solid rgba\(255,59,213,\.95\)/);
  assert.match(classicCurrentRule, /0 0 4px rgba\(255,59,213,\.72\)[\s\S]*0 0 11px rgba\(255,59,213,\.4\)/);
...
test("hand results stay on the table without a popup and advance automatically", () => {
  ...
  assert.match(classicWinnerRule, /border:\s*1px solid #FF3BD5/);
  assert.match(classicWinnerRule, /0 0 5px rgba\(255,59,213,\.96\)[\s\S]*0 0 16px rgba\(255,59,213,\.58\)/);
