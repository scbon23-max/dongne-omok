...
test("the six-seat table exposes every required game control", () => {
  ...
  assert.match(controller, /function heroCurrentHand\(\)[\s\S]*var isHighCard = Number\(evaluation\.category\) === 0[\s\S]*if \(isHighCard\)[\s\S]*holeCards\[index\] = true[\s\S]*if \(!isHighCard && code && bestCards\[code\]\) communityCards\[index\] = true/);
  ...
  assert.match(styles, /\.holdem-board \.holdem-card\.is-hero-made-hand-card,[\s\S]*\.holdem-hole-cards \.holdem-card\.is-hero-made-hand-card\s*\{[\s\S]*inset 0 0 0 1px/);
