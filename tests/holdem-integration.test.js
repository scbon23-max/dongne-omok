...
test("the betting UI keeps raise choices collapsed until requested", () => {
  ...
  assert.match(styles, /\.holdem-seat-turn-timer\s*\{[\s\S]*width:\s*var\(--holdem-seat-avatar-size\)/);
  assert.match(styles, /\.holdem-seat-turn-timer::before\s*\{[\s\S]*conic-gradient\(#4df5d9 calc\(var\(--holdem-seat-timer-ratio\) \* 1turn\)/);
  assert.match(styles, /\.holdem-seat-turn-timer::after\s*\{[\s\S]*content:\s*attr\(data-seconds\)[\s\S]*inset:\s*0/);
  ...
  assert.match(styles, /\.holdem-screen\.is-actioning \.holdem-quick-bets button\[data-holdem-bet="allin"\] \{[\s\S]*grid-column: 1[\s\S]*grid-row: 3/);
  assert.match(styles, /\.holdem-screen\.is-actioning \.holdem-quick-bets button\[data-holdem-bet="eight-pot"\] \{ grid-row: 1; \}/);
  assert.match(styles, /\.holdem-screen\.is-actioning \.holdem-quick-bets button\[data-holdem-bet="four-pot"\] \{ grid-row: 2; \}/);
...
test("hand results stay on the table without a popup and advance automatically", () => {
  ...
  assert.match(styles, /\.holdem-win-gain\s*\{[\s\S]*font-size:\s*clamp\(16px,\s*4\.8vw,\s*23px\)[\s\S]*font-weight:\s*1000/);
