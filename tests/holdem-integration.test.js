...
test("the six-seat table exposes every required game control", () => {
  ...
  assert.match(controller, /function heroCurrentHand\(\)[\s\S]*relevantBestCardCodes\(evaluation\)[\s\S]*if \(code && bestCards\[code\]\) communityCards\[index\] = true/);
  assert.doesNotMatch(controller, /currentHand && currentHand\.holeCards\[cardIndex\] \? "is-hero-made-hand-card" : ""/);
  ...
  assert.doesNotMatch(styles, /\.holdem-hole-cards \.holdem-card\.is-hero-made-hand-card/);
  ...
  assert.match(styles, /\.holdem-screen\[data-card-front-skin="four-color"\] \.holdem-card\.is-winning-combo-card:not\(\.back\)\s*\{[\s\S]*inset 0 0 0 1px rgba\(255,225,109,\.82\)[\s\S]*0 0 7px rgba\(255,207,72,\.34\)/);
...
test("hand results stay on the table without a popup and advance automatically", () => {
  ...
  assert.match(styles, /\.holdem-screen\[data-card-front-skin="four-color"\] \.holdem-card\.is-winning-combo-card:not\(\.back\)\s*\{[\s\S]*inset 0 0 0 1px rgba\(255,225,109,\.82\)/);
