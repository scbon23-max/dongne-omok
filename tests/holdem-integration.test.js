...
test("the eight-seat table exposes every required game control", () => {
  ...
  assert.match(index, /<nav class="holdem-utility"[\s\S]*class="holdem-exit-stack"[\s\S]*id="holdem-leave-btn"[\s\S]*id="holdem-spectator-chip"[\s\S]*id="holdem-history-btn"[\s\S]*id="holdem-hands-btn"[\s\S]*id="holdem-settings-btn"/);
  assert.match(index, /id="holdem-history-btn"[\s\S]*aria-expanded="false"[\s\S]*>🕘<\/button>/);
  assert.match(index, /id="holdem-history-panel"[\s\S]*id="holdem-history-list"[\s\S]*id="holdem-history-detail"/);
  assert.match(styles, /\.holdem-topbar\s*\{[\s\S]*background:\s*transparent/);
  assert.match(styles, /\.holdem-utility \.holdem-history-btn\s*\{[\s\S]*width:\s*34px/);
  assert.match(styles, /\.holdem-history-layout\s*\{[\s\S]*grid-template-rows:\s*auto minmax\(0, 1fr\)/);
  assert.match(styles, /@media \(min-width: 900px\)[\s\S]*\.holdem-history-layout\s*\{[\s\S]*grid-template-columns:\s*205px minmax\(0, 1fr\)/);
  ...
  assert.match(controller, /function normalizeHandHistory\(value\)[\s\S]*showdown:[\s\S]*normalizeHandHistoryShowdown/);
  assert.match(controller, /function renderHandHistory\(\)[\s\S]*state\.handHistory\.slice\(\)\.reverse\(\)/);
  ...
  assert.match(controller, /function triplePlusMadeHandKey\(snapshot\)[\s\S]*Number\(evaluation\.category\) < 3[\s\S]*var bestCards = relevantBestCardCodes\(evaluation\)[\s\S]*if \(!usesHeroCard\) return ""[\s\S]*snapshotHandKey\(snapshot\),[\s\S]*"made",[\s\S]*Number\(evaluation\.category\) \|\| 0/);
...
test("the betting UI keeps raise choices collapsed until requested", () => {
  ...
  assert.match(controller, /moneyUnitMode === "bb"[\s\S]*amount \/ state\.bigBlind[\s\S]*storeQueuedProfileTopUp\(amount, \{ bb: queuedBb \}\)/);
  assert.doesNotMatch(controller, /hero\.stack <= 0 && state\.canRefill/);
  ...
  assert.match(styles, /\.holdem-screen\.is-good-hand-cue \.holdem-seat\.is-me \.holdem-hole-cards \.holdem-card:not\(\.back\):not\(\.empty\)\s*\{[\s\S]*will-change:\s*filter/);
  assert.match(styles, /\.holdem-screen\.is-good-hand-cue \.holdem-seat\.is-me \.holdem-hole-cards \.holdem-card:not\(\.back\):not\(\.empty\)::before,[\s\S]*animation:\s*holdemGoodHandShineSweep \.86s/);
  assert.doesNotMatch(styles, /\.holdem-screen\.is-good-hand-cue \.holdem-action-buttons/);
  assert.doesNotMatch(styles, /\.holdem-screen\.is-good-hand-cue #holdem-pre-action-panel/);
...
test("hand results stay on the table without a popup and advance automatically", () => {
  ...
  assert.doesNotMatch(engine, /function foldPlayerForSpectate\(/);
  assert.match(controller, /var reserveAfterHand = !!\(hero && isHandActive\(state\.phase\) && hero\.inHand\)/);
  assert.doesNotMatch(controller, /heroPrivateViewBlocked/);
  ...
  assert.match(engine, /function startHand\(state, now, context\)[\s\S]*state\.seats\.filter\(canPlayNextHand\)/);
...
test("Hold'em chat keeps transient toasts separate from the recent typing history", () => {
  ...
  assert.doesNotMatch(game, /var holdemHistCount = \{\}/);
  assert.doesNotMatch(game, /var holdemMerged = \[\]/);
  assert.doesNotMatch(game, /holdemMerged\.push\(\{ game: "holdem", who: m\.nick, text: m\.text \}\)/);
  assert.match(game, /var recent = \[\]/);
  assert.doesNotMatch(game, /recent\.length < \d+/);
  assert.match(game, /row\.game !== "holdem"/);
  assert.doesNotMatch(game, /row\.who === "__sys"/);
  assert.match(game, /var shouldStickToBottom = !hadHistory \|\| distanceFromBottom <= 12/);
  assert.match(game, /historySurface\.scrollTop = shouldStickToBottom \? historySurface\.scrollHeight : previousScrollTop/);
