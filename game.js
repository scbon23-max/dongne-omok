(function () {
  ...
  var HOLDEM_BUY_IN_OPTIONS = [
    { amount: 15000, title: "라이트", minBuyIn: 10000, maxBuyIn: 20000, smallBlind: 100, bigBlind: 200 },
    { amount: 30000, title: "스탠다드", minBuyIn: 20000, maxBuyIn: 40000, smallBlind: 200, bigBlind: 400 },
  ...
  function readStoredRoomLease() {
    ...
    return {
      ...
      buyIn: saved.game === "holdem_ring"
        ? normalizeHoldemBuyIn(saved.buyIn)
        : 0,
  ...
  function storeRoomLease(lease) {
    ...
    try {
      sessionStorage.setItem(ROOM_LEASE_STORAGE_KEY, JSON.stringify({
        ...
        game: lease.game,
        buyIn: lease.game === "holdem_ring"
          ? normalizeHoldemBuyIn(lease.buyIn)
          : 0,
        token: lease.token,
  ...
  async function restoreOwnedRoomLease() {
    ...
    if (lease.game === "holdem_ring") {
      lease.buyIn = normalizeHoldemBuyIn(claimed.buyIn, lease.buyIn);
    }
    roomLease = lease;
    if (curRoomId) {
  ...
  async function createRoom(game, name, options) {
    options = options || {};
    if (!canEnterGame(game)) {
      toast(game === "catchmind" ? "캐치마인드는 점검 중이라 이용할 수 없어요" : "아직 테스트 중인 게임이에요");
      return;
    ...
    var lease = {
      ...
      roomName: title,
      game: game,
      buyIn: game === "holdem_ring"
        ? normalizeHoldemBuyIn(options.buyIn)
        : 0,
      token: roomLeaseToken()
    };
  ...
  function drawStoneShadow(x, y, color) {
    ctx.save();
  ...
  function renderCreateGameOptions(selected) {
    ...
    box.innerHTML = ids.map(function (id) {
      ...
      var iconSrc = id === "omok" ? "assets/create-room-icons/omok.webp"
        : id === "alk" ? "assets/create-room-icons/alk.webp"
        : id === "catchmind" ? "assets/create-room-icons/catchmind.webp"
        : id === "holdem" ? "assets/create-room-icons/holdem.webp"
      ...
      var icon = '<img class="' + iconClass + '" src="' + localAssetUrl(iconSrc) + '" alt="" width="78" height="78" decoding="async">';
  ...
  function normalizeHoldemAssetRankingDetail(value) {
    ...
    return {
      ...
      refillCountBaselineDate: String(value.refillCountBaselineDate || "").trim().slice(0, 20),
  ...
  function setHoldemAssetRankingView(view) {
    var dialog = document.querySelector(".holdem-asset-ranking-dialog");
    var detailMode = view === "detail";
    if (dialog) dialog.classList.toggle("is-detail", detailMode);
    if ($("holdem-asset-ranking-title")) {
      $("holdem-asset-ranking-title").textContent = detailMode ? "플레이어 기록" : "자산 랭킹";
    }
  }
  function backToHoldemAssetRankingList() {
    setHoldemAssetRankingView("list");
  ...
  function holdemAssetRankingBackHtml() {
    return '<div class="holdem-asset-ranking-detail-nav">' +
      '<button type="button" data-holdem-ranking-back>← 랭킹</button>' +
      '<span>상세 기록</span></div>';
  }
  function renderHoldemAssetRankingDetail(detail) {
    ...
    if (!box) return;
    setHoldemAssetRankingView("detail");
    if (!detail) {
      box.classList.remove("hidden");
    ...
    var ledgerNote = '<div class="holdem-asset-ranking-ledger-note">' +
      ...
      (detail.refillCountBaselineDate ? ' (' + esc(detail.refillCountBaselineDate) + ' 기준)' : "") +
    ...
    box.innerHTML = holdemAssetRankingBackHtml() +
      ...
      holdemAssetRankingPositionHtml(detail.rank) +
      '<div><small>PLAYER</small><strong>' + esc(detail.nickname) + '</strong>' +
      '<em>' + esc(formatHoldemAsset(detail.totalAssets)) + '</em></div></div>' +
      ...
      '<span><small>무료충전 횟수</small><strong>' + esc(detail.refillCount + "회") + '</strong></span>' +
  ...
  function showHoldemAssetRankingDetailStatus(message) {
    ...
    box.classList.remove("hidden");
    box.innerHTML = holdemAssetRankingBackHtml() +
      '<p class="holdem-asset-ranking-status">' + esc(message) + '</p>';
  }
  ...
  function renderHoldemWalletControls(mode) {
    var availableBalance = holdemWalletProfile &&
      ...
      ? Math.max(0, Math.floor(Number(holdemWalletProfile.balance)))
      : null;
    var totalAssets = holdemWalletProfile &&
      Number.isFinite(Number(holdemWalletProfile.totalAssets))
      ? Math.max(0, Math.floor(Number(holdemWalletProfile.totalAssets)))
      : availableBalance;
    var tableBalance = holdemWalletProfile &&
    ...
    if ($("create-holdem-wallet-status")) {
      $("create-holdem-wallet-status").textContent = holdemWalletPending
  ...
  async function loadHoldemWallet(force) {
    ...
    if (!force && (
      holdemWalletPending ||
      (holdemWalletProfile && holdemWalletNick === requestedNick)
    )) {
      renderHoldemWalletControls(activeCreateHoldemMode);
      return holdemWalletProfile;
    }
    if (!window.Db || !Db.getHoldemWallet || !sessionAuthHash) {
      holdemWalletProfile = null;
      holdemWalletNick = "";
      renderHoldemWalletControls(activeCreateHoldemMode);
      return null;
    }
    var requestSeq = ++holdemWalletRequestSeq;
    if (holdemWalletNick !== requestedNick) {
      holdemWalletProfile = null;
      holdemWalletNick = "";
    }
    holdemWalletPending = true;
    renderHoldemWalletControls(activeCreateHoldemMode);
    var result;
    try {
      result = await Db.getHoldemWallet(roomLeaseAuth());
    } catch (error) {
      result = null;
    }
    if (requestSeq !== holdemWalletRequestSeq) {
      return null;
    }
    if (requestedNick !== String(me.nick || "")) {
      holdemWalletPending = false;
      holdemWalletProfile = null;
      holdemWalletNick = "";
      renderHoldemWalletControls(activeCreateHoldemMode);
      return null;
    ...
    holdemWalletPending = false;
    var wallet = result && result.ok && result.wallet &&
  ...
  var omokSolo = false;
  var omokAI = { on: false, level: null, color: WHITE };
  var aiPending = false;
  var AI_NICK = "AI";
  ...
  function aiHumanSeatColor(seats, nick) {
    ...
    if (seats.white === nick) return "white";
    return null;
  }
  ...
  function bind() {
    ...
    $("holdem-asset-ranking-detail").addEventListener("click", function (event) {
      if (event.target.closest("[data-holdem-ranking-back]")) backToHoldemAssetRankingList();
    });
